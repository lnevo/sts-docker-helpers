...
resolve_sts_docker
if [[ -f "${SCRIPT_DIR}/tune_shipment_orders.sql" ]]; then
  SQL_FILE="${SCRIPT_DIR}/tune_shipment_orders.sql"
elif [[ -f "${SCRIPT_DIR}/migrations/tune_shipment_orders.sql" ]]; then
  SQL_FILE="${SCRIPT_DIR}/migrations/tune_shipment_orders.sql"
else
  SQL_FILE="${SCRIPT_DIR}/tune_shipment_orders.sql"
fi
YARD_BALANCE_PY="${SCRIPT_DIR}/balance_shipment_yards.py"
...
if [[ "${SQL_FILE}" == "${SCRIPT_DIR}/tune_shipment_orders.sql" && -f "${YARD_BALANCE_PY}" ]]; then
  echo "==> Balancing interchange yards (available fleet)"
  python3 "${YARD_BALANCE_PY}" --apply --config "${SCRIPT_DIR}/hart_seed_config.json"
fi
